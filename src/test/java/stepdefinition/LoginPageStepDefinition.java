...
public class LoginPageStepDefinition {
	...
	public void user_should_see_the_text_on_the_left_side_of_navigation_bar(String pageHeading) {
		...
		Assert.assertEquals(actualHeading, pageHeading, "Page heading text does not match expected value.");
	}
	...
	public void username_and_password_labels_should_be_left_aligned_above_their_respective_input_fields() {
		Assert.assertTrue(pom.getLoginPage().isLebelsleftalignedaboveInputField(),
				"Labels are not left-aligned above their respective input fields.");
	}

	@Then("User should see exactly two input field")
	public void user_should_see_exactly_two_input_field() {
		Assert.assertEquals(pom.getLoginPage().getLabelCount(), 2,
				"The number of input fields does not match the expected count of 2.");
	}
